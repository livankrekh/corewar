...
void	getImageBackground(SDL_Renderer *render, SDL_DisplayMode displayMode)
{
	...
	if ((BMP_back = SDL_LoadBMP("8bit.bmp")) == NULL)
		ft_putstr("Image cannot download!\n");
	if ((back = SDL_CreateTextureFromSurface(render, BMP_back)) == NULL)
		ft_putstr("Create texture error!\n");
