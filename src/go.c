/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   go.c                                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: liabanzh <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/07/18 17:55:40 by liabanzh          #+#    #+#             */
/*   Updated: 2017/07/18 17:55:41 by liabanzh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/vm.h"
#include "../includes/op.h"
#include <unistd.h>

void	delete_split(char ***split)
{
	int		i;

	i = 0;
	while (*split[i] != NULL)
	{
		free(*split[i]);
		*split[i] = NULL;
		i++;
	}
	free(*split);
	*split = NULL;
}

byte	*get_map(t_players *players, int count)
{
	unsigned char	*map;
	int				i;
	int				j;
	char			*ptr;
	char			*tmp;

	j = 1;
	i = 0;
	map = (unsigned char*)malloc(sizeof(unsigned char) * MEM_SIZE);
	ptr = players[0].comands;
	while (i < MEM_SIZE && j <= count)
	{
		if (i >= (MEM_SIZE / count) * j)
		{
			ptr = players[j].comands;
			j++;
		}
		if (*ptr != '\0')
		{
			tmp = ft_strnew(2);
			map[i] = ft_atoi_base(ft_strncpy(tmp, ptr, 2), 16);
			ptr += 2;
			free(tmp);
		}
		else
			map[i] = 0;
		i++;
	}
	return (map);
}

void	get_command(t_players *players, byte *map)
{
	int i;

	i = 0;
	while (players[i].header.name != NULL)
	{
		if (map[players[i].pos] == 1)
		{
			live(players, map[players[i].pos + 1]);
			players[i].pos += 1;
		}
		else if (map[players[i].pos] == 9)
			zjmp(&(players[i]), map);
		else if (map[players[i].pos] == 11)
			sti(&(players[i]), map);
		else
		{
			players[i].pos += 1;
			players[i].pos = players[i].pos % MEM_SIZE;
		}
		i++;
	}
	map[2] = map[2];
}

void	go_vm(t_players *players, int count)
{
	byte	*map;
	int		i;
	WINDOW	*win;
	WINDOW	*win1;
	WINDOW	*win2;

	i = 0;
	win1 = NULL;
	win2 = NULL;
	win = initscr();
	map = get_map(players, count);
	vizualize(map, players, &win1, win);
	status_bar(&win2, win, players);
	curs_set(0);
	getch();
	cursor_refresh(win1, win2, players, map);
	getch();
	while (1)
	{
		get_command(players, map);
		cursor_refresh(win1, win2, players, map);
		//sleep(1);
	}
	delwin(win1);
	delwin(win2);
	delwin(win);
	endwin();
}

int		main(void)
{
	t_players		players[5];

	players[0].header.name = "zork";
	players[0].comands = "0b681300070001010000000109fffb";
	players[0].num = -1;
	players[0].reg = (unsigned char*)malloc(sizeof(unsigned char) * 16);
	players[0].live = 0;
	ft_bzero(players[0].reg, 16);
	players[0].reg[0] = players[0].num;
	players[1].header.name = "helltrain";
	players[1].comands = "";
	players[1].num = -2;
	players[1].live = 0;
	players[1].reg = (unsigned char*)malloc(sizeof(unsigned char) * 16);
	ft_bzero(players[1].reg, 16);
	players[1].reg[0] = players[1].num;
	players[2].header.name = "skynet";
	players[2].comands = "";
	players[2].num = -3;
	players[2].live = 0;
	players[2].reg = (unsigned char*)malloc(sizeof(unsigned char) * 16);
	ft_bzero(players[2].reg, 16);
	players[2].reg[0] = players[2].num;
	players[3].header.name = "Asombra";
	players[3].comands = "";
	players[3].num = -4;
	players[3].live = 0;
	players[3].reg = (unsigned char*)malloc(sizeof(unsigned char) * 16);
	ft_bzero(players[3].reg, 16);
	players[3].reg[0] = players[3].num;
	players[4].header.name = NULL;
	players[4].comands = NULL;
	go_vm(players, 4);
	return (0);
}
